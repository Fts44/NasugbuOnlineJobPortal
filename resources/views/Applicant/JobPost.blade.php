            ...
            <div class="modal-body">
                <form action="" id="job_post_form">
                    ...
                    <label class="form-control border-0 p-0 mb-3">
                        ...
                        <span class="text-danger job_post-error" id="resume-error"></span>
                    </label>
                </form>  
            </div>
...
<script>
    ...
    $('#apply').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: "{{ route('JobPostApplicantApply') }}",
            contentType: false,
            processData: false,
            data: formData,
            success: function(response){
    ...
    $('#resume').change(function(){
        ...
        if (fileSize > MAX_FILE_SIZE) {
            $('#resume-error').html("File must no be greater than 5mb!");
            $(this).val('');
        } 
        else {
            var allowed_extensions = new Array("pdf", "jpg");
            ...
            if((allowed_extensions.includes(file_extension))==true){
                ...
                $('#resume-error').html("Valid file type are pdf and jpg!");
                $(this).val('');
            }
        }
    });
